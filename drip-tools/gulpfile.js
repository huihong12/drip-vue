...
function tag() {
  ...
  execSync(
    `git push https://${process.env.GITHUB_TOKEN}@github.com/huihong12/drip-vue.git ${version}:${version}`,
  );
  execSync(
    `git push https://${process.env.GITHUB_TOKEN}@github.com/huihong12/drip-vue.git master:master`,
  );
  console.log('tagged');
}
...
function publish(tagString, done) {
  let args = ['publish', '--with-drip-tools'];
  if (tagString) {
  ...
  gulp.series(done => {
    ...
    if (npmArgs) {
      for (let arg = npmArgs.shift(); arg; arg = npmArgs.shift()) {
        if (/^pu(b(l(i(sh?)?)?)?)?$/.test(arg) && npmArgs.indexOf('--with-drip-tools') < 0) {
          reportError();
          done(1);
          return;
        }
